import type { InternalMonitoring, Observable, MonitoringMessage } from '@datadog/browser-core'
import { combine, canUseEventBridge, startInternalMonitoring, getEventBridge } from '@datadog/browser-core'
import { startMonitoringBatch } from '@datadog/browser-core/src/domain/internalMonitoring/startMonitoringBatch'
...
export function startRum(
  ...
  if (canUseEventBridge()) {
    session = startRumSessionManagerStub()
    startRumEventBridge(lifeCycle)
  } else {
    session = startRumSessionManager(configuration, lifeCycle)
...
function startRumInternalMonitoring(configuration: RumConfiguration): InternalMonitoring {
